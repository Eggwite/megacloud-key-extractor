...
function evaluateAstNode(node, args) {
  if (!node)
    throw new Error("evaluateAstNode received a null or undefined node.");
  ...
  throw new Error(
    `Unsupported node type in evaluation: ${node.type} (${gen(node)})`
  );
}
...
export const solveStateMachine = {
  visitor: {
    ...
    AssignmentExpression(path, state) {
      ...
      const returnArg =
        body.length === 1 && body[0].isReturnStatement()
          ? body[0].get("argument")
          : null;

      if (
        !returnArg ||
        !returnArg.isObjectExpression() ||
        returnArg.get("properties").length !== 2
      ) {
        return;
      }

      const stateVarName =
        callee.node.params.length === 1 ? callee.node.params[0].name : null;
      if (!stateVarName) {
        return;
      }
      ...
      for (const prop of returnArg.get("properties")) {
        ...
        const propName = key.isIdentifier()
          ? key.node.name
          : key.isStringLiteral()
          ? key.node.value
          : null;
