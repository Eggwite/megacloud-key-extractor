  ...
  visitor: {
    ForStatement: {
      exit(path) {
        ...
        if (!t.isIdentifier(stateVar) && !t.isMemberExpression(stateVar))
          return;
        ...
        if (
          !t.isSwitchStatement(switchStatement) ||
          !t.isNodesEquivalent(switchStatement.discriminant, stateVar)
        )
          return;

        debug.log(
          `[UNFLATTEN] Found state machine with terminal value: ${terminalValue}`
        );
        ...
        for (let i = path.key - 1; i >= 0; i--) {
          ...
          if (siblingPath.isVariableDeclaration()) {
            const declarator = siblingPath.node.declarations.find(
              (d) =>
                t.isNodesEquivalent(d.id, stateVar) &&
                d.init &&
                t.isNumericLiteral(d.init)
            );
            if (declarator) {
            ...
            if (
              ...
              t.isNodesEquivalent(expr.left, stateVar) &&
        ...
        debug.log(`[UNFLATTEN] Initial state: ${initialState}`);
        ...
        debug.log(`[UNFLATTEN] Found ${caseMap.size} switch cases`);
        ...
        function unflatten(currentState, recursionStack) {
          ...
          if (
            ...
            t.isAssignmentExpression(lastStmt.expression, { operator: "=" }) &&
            t.isNodesEquivalent(lastStmt.expression.left, stateVar)
          ) {
            stateUpdateRhs = lastStmt.expression.right;
            bodyWithoutUpdate = caseBody.slice(0, -1);
          }

          const processedBody = processVarDeclarations(
            bodyWithoutUpdate,
            hoistedVars
          );
          ...
          if (!stateUpdateRhs) {
            ...
            const nextBlock = unflatten(
              stateUpdateRhs.value,
              newRecursionStack
            );
            ...
            if (
              trueBranchBody.length === 0 &&
              newRecursionStack.has(consequent.value)
            ) {
              const whileStatement = t.whileStatement(
                test,
                t.blockStatement(processedBody)
              );
        ...
        if (success) {
          ...
          debug.log(
            `[UNFLATTEN] Successfully unflattened state machine. Generated ${newNodes.length} nodes.`
          );
          ...
          debug.log("[UNFLATTEN] Failed to unflatten state machine");
        }
      },
    },
  },
